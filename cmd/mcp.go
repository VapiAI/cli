...
func displayPostSetupInstructions() {
	...
	fmt.Println("• \"Find MCP server documentation for Claude Desktop configuration\"")
	fmt.Println("• \"Show me Vapi examples for creating voice assistants\"")
	fmt.Println("• \"How do I set up phone numbers and make outbound calls?\"")
	fmt.Println("• \"What's the difference between function tools and MCP tools?\"")
	fmt.Println("• \"Show me webhook configuration examples\"")
	fmt.Println("• \"Find the latest Vapi API reference for assistants\"")
	fmt.Println()
	fmt.Println("Your IDE AI assistant now has access to:")
	fmt.Println("• 138+ real documentation pages with actual content")
	fmt.Println("• MCP server setup guides and Claude Desktop configuration")
	fmt.Println("• Complete API reference with request/response examples")
	fmt.Println("• Voice provider configurations (ElevenLabs, OpenAI, etc.)")
	fmt.Println("• Real code examples from the official documentation")
	fmt.Println("• Integration guides for tools, webhooks, and workflows")
	fmt.Println()
	fmt.Println("💡 Pro tip: Use 'Find Vapi examples for [feature]' to get real code samples!")
	fmt.Println()
